<x-app-layout>
    <x-slot name="header">
        ...
        <p class="text-gray-600 text-sm mt-1">Kelola semua transaksi barang masuk</p>
    </x-slot>
    ...
    <div class="min-h-screen bg-gradient-to-br from-green-50 via-white to-indigo-50 py-12">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <form action="{{ route('barang-masuk.index') }}" method="GET" class="md:col-span-2 flex gap-3">
                    <div class="relative flex-1">
                        <input type="text" name="search" 
                            placeholder="Cari berdasarkan keterangan, supplier, kategori atau nama barang"
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-gradient-to-r from-green-600 to-green-500 text-white">
                                ...
                                <th class="px-6 py-4 text-left font-semibold text-sm">Kode Barang</th>
                                ...
                                <th class="px-6 py-4 text-left font-semibold text-sm">Harga Satuan</th>
                                <th class="px-6 py-4 text-left font-semibold text-sm">Supplier</th>
                            ...
                            @forelse ($barangMasuk as $index => $masuk)
                                <tr class="hover:bg-gray-50 transition-colors duration-150 group">
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                        {{ $barangMasuk->currentPage() * 10 - 10 + $index + 1 }}
                                    </td>

                                    <td class="px-6 py-4">
                                        <div class="flex justify-start">
                                            <div class="relative inline-block">
                                                <img src="{{ $masuk->item->gambar ?? 'https://www.shutterstock.com/image-vector/default-ui-image-placeholder-wireframes-600nw-1037719192.jpg' }}"
                                                    alt="{{ $masuk->item->nama_barang }}"
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-semibold text-gray-900">{{ $masuk->kode_barang }}</p>
                                    </td>

                                    <td class="px-6 py-4">
                                        <p class="text-sm font-semibold text-gray-900">{{ $masuk->item->nama_barang }}</p>
                                        <p class="text-xs text-gray-500">{{ $masuk->item->kode_barang }}</p>
                                    </td>

                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ $masuk->jumlah }} {{ $masuk->item->satuan }}
                                        </span>
                                    </td>

                                    <td class="px-6 py-4 text-sm">
                                        @if ($masuk->kategori === 'pembelian')
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">{{ $masuk->nama_supplier }}</td>

                                    <td class="px-6 py-4 text-sm text-gray-600">
                                        <span class="line-clamp-2">{{ $masuk->keterangan }}</span>
                                    </td>

                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('barang-masuk.edit', $masuk->masuk_id) }}"
                                            ...
                                            <button x-data 
                                                @click="$dispatch('open-modal', 'delete_masuk_{{ $masuk->masuk_id }}')"
                                            ...
                                            <x-confirm-modal id="delete_masuk_{{ $masuk->masuk_id }}"
                                                message="Apakah Anda yakin ingin menghapus transaksi barang masuk ini? Tindakan ini tidak dapat dibatalkan."
                                                okLabel="Hapus" cancelLabel="Batal" 
                                                :url="route('barang-masuk.destroy', $masuk->masuk_id)" method="DELETE" />
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="10" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-sm text-gray-400 mb-6">Belum ada transaksi barang masuk</p>
                                            <a href="{{ route('barang-masuk.create') }}"
