    ...
    <div class="min-h-screen bg-gradient-to-br from-green-50 via-white to-indigo-50 py-12">
        <div class="max-w-2xl mx-auto px-6">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <!-- Form Content -->
                <div class="p-8">
                    <form id="update_item" method="POST" action="{{ route('items.update', $item->item_id) }}"
                        ...
                        <!-- Template Selection -->
                        <div class="group">
                            <x-input-label for="item_templates_id" :value="__('Kategori Template (Opsional)')"
                                class="text-gray-700 font-semibold mb-2 flex items-center gap-2" />
                            <div class="relative">
                                <select id="item_templates_id" name="item_templates_id"
                                    ...
                                    @foreach ($itemTemplates as $template)
                                        <option value="{{ $template->item_templates_id }}"
                                            {{ old('item_templates_id', $item->item_templates_id) == $template->item_templates_id ? 'selected' : '' }}>
                                            {{ $template->kategori }} - {{ $template->jenis }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <p class="mt-2 text-sm text-gray-500">Pilih kategori template atau biarkan kosong untuk item
                                tanpa kategori</p>
                            <x-input-error :messages="$errors->get('item_templates_id')" class="mt-2" />
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="group">
                                ...
                                <x-text-input id="kode_barang"
                                    class="block w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-green-500 focus:ring-green-500 transition-all duration-200 hover:border-gray-300 bg-gray-50 focus:bg-white"
                                    type="text" name="kode_barang" :value="old('kode_barang', $item->kode_barang)" placeholder="Contoh: BRG-001"
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <select id="kondisi" name="kondisi" required
                                        class="block w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-green-500 focus:ring-green-500 transition-all duration-200 hover:border-gray-300 bg-gray-50 focus:bg-white appearance-none">
                                        <option value="baik"
                                            {{ old('kondisi', $item->kondisi) == 'baik' ? 'selected' : '' }}>Baik
                                        </option>
                                        <option value="diperbaiki"
                                            {{ old('kondisi', $item->kondisi) == 'diperbaiki' ? 'selected' : '' }}>
                                            Diperbaiki</option>
                                        <option value="dipinjam"
                                            {{ old('kondisi', $item->kondisi) == 'dipinjam' ? 'selected' : '' }}>
                                            Dipinjam</option>
                                    </select>
                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor"
                            ...
                            <div class="relative">
                                <label
                                    ...
                                    <span id="fileName" class="text-gray-500 text-sm font-medium">
                                        Pilih file gambar baru (jpg, jpeg, png)
                                    </span>
                                ...
                                <input id="gambar" class="hidden" type="file" name="gambar"
                                    accept="image/*" />
                            </div>

                            <!-- Image Preview -->
                            <div id="cover_preview_wrapper" class="mt-4 {{ $item->gambar_url ? '' : 'hidden' }}">
                                <div class="bg-gray-50 rounded-xl p-4 border-2 border-gray-200">
                                    ...
                                    <div class="relative inline-block">
                                        <img id="cover_preview" src="{{ $item->gambar_url ?? '' }}"
                                            alt="Image Preview"
                        ...
                        <div class="flex items-center justify-end pt-6 border-t border-gray-200 space-x-4">
                            ...
                            <x-confirm-modal id="update_confirmation"
                                ...
                                cancelLabel="Batal" :url="route('items.update', $item->item_id)" formId="update_item" />
                        </div>
                    </form>
                </div>
            </div>
