            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach ($barang as $index => $item)
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($item->kode_barang)
                                                <span
                                                    ...
                                                    {{ $jenisBarang->kode_utama . '' . $item->kode_barang }}
                                                </span>
                                            @else
                                        ...
                                        @hasrole('admin')
                                            <td class="px-6 py-4">
                                                <div class="flex items-center justify-center gap-2">
                                                    <a href="{{ route('barang.edit', [$jenisBarang->jenis_barang_id, $item->barang_id]) }}"
                                                    ...
                                                    <button type="button" x-data
                                                        @click="$dispatch('open-modal', 'delete_barang_{{ $item->barang_id }}')"
                                    ...
                                    <x-confirm-modal id="delete_barang_{{ $item->barang_id }}"
                                        message="Apakah Anda yakin ingin menghapus barang '{{ $item->nama_barang }}'?"
                                        okLabel="Hapus" cancelLabel="Batal" :url="route('barang.destroy', $item->barang_id)" method="DELETE" />
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="text-sm text-gray-400 mb-6">Jenis barang ini belum memiliki item</p>
                        @hasrole('admin')
                            <a href="{{ route('barang.create', $jenisBarang->jenis_barang_id) }}?template_id={{ $jenisBarang->jenis_barang_id }}"
